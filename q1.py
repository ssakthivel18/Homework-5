...
Original file is located at
    https://colab.research.google.com/drive/1ec-J440Vfe1TsV7KH_QZC4XSnlCyPODX
"""
...
def relu_backward(X):
    ...
    """
    Z = np.maximum(0, X)
    dZdX = np.zeros_like(X)  # create a matrix of zeros with the same shape as X
    dZdX[Z > 0] = 1         # set elements where X > 0 to 1
...
def softmax_backward(X):
    ...
    delta = np.eye(D)
    dZ_dX = np.zeros((N, D, D))
    for n in range(N):
        for i in range(D):
            for j in range(D):
                dZ_dX[n, i, j] = Z[n, i] * (delta[i, j] - Z[n, j])
